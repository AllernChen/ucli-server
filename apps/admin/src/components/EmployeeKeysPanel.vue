...
function selectGroup() {
  ...
  if (group?.type === 'REGION') void loadEmployeeProjects(form.accountId || props.accountId || '', form.groupId)
  else { projects.value = []; form.projectId = '' }
}
async function save() {
  const accountId = managed.value ? form.accountId : props.accountId
  const regionKey = groups.value.find(item => item.id === form.groupId)?.type === 'REGION'
  if (pending.value || !adminMode.value || !accountId || !form.name.trim() || !form.groupId || (regionKey && !form.projectId)) return
  const current = viewGeneration; pending.value = true; formError.value = ''
  try {
    const body = { name: form.name.trim(), ...(!editing.value ? { groupId: form.groupId, projectId: form.projectId || null } : {}), expiresAt: form.expiresAt ? new Date(form.expiresAt).toISOString() : null }
