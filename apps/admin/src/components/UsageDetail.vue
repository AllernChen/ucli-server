...
const loading = ref(false); const error = ref(''); const detail = ref<any>(null); const copied = ref(false)
...
async function load() {
  ...
  const current = lifecycle.next(); loading.value = true; error.value = ''; detail.value = null
...
async function copyRequestId() { if (detail.value?.requestId) { await navigator.clipboard?.writeText(detail.value.requestId); copied.value = true } }
