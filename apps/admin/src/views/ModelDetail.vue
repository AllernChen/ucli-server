...
<template>
  ...
  <template v-if="model && !loading">
    ...
    <section class="panel"><div class="section-header"><div><h2>渠道实际采购价格</h2><p class="muted">与渠道模块使用同一份价格数据：渠道分时规则优先，未命中时使用下方公共模型兜底价。</p></div></div><table v-if="model.abilities.length"><thead><tr><th>渠道</th><th>上游模型 / 协议</th><th>健康</th><th>当前生效价格</th><th>分时价格</th><th>操作</th></tr></thead><tbody><tr v-for="ability in model.abilities" :key="ability.id"><td>{{ channelName(ability.channelId) }}</td><td><span class="mono">{{ ability.upstreamModel }}</span><small>{{ ability.protocol }}</small></td><td><StatusBadge :status="ability.health" /></td><td><strong>{{ effectiveChannelCost(ability.currentCost).priceLabel }}</strong><small>{{ effectiveChannelCost(ability.currentCost).sourceLabel }}</small></td><td><div v-if="ability.costRules?.length" class="rule-list"><div v-for="rule in ability.costRules" :key="rule.id"><strong>{{ rule.name }} · 输入 {{ formatCny(rule.inputPerMillion) }} / 输出 {{ formatCny(rule.outputPerMillion) }} / M</strong><small>{{ scheduledCostLabel(rule, ability.costTimezone) }} · 优先级 {{ rule.priority }}{{ rule.enabled ? '' : ' · 已停用' }}</small></div></div><span v-else class="muted">未设置渠道规则</span></td><td><button @click="router.push({ path: `/channels/${ability.channelId}`, query: { tab: 'models' } })">打开渠道价格</button></td></tr></tbody></table><p v-else class="empty">尚无渠道供应映射，请前往渠道详情添加。</p></section>
