...
<div class="table-panel"><table><thead><tr><th>名称</th><th><button @click="changeSort('requests')">请求数</button></th><th><button @click="changeSort('tokens')">Token</button></th><th><button @click="changeSort('costUsd')">采购成本</button></th><th><button @click="changeSort('successRate')">成功率</button></th><th><button @click="changeSort('p95LatencyMs')">P95</button></th><th v-if="dimension === 'costRule'">加权采购单价 / 时段</th></tr></thead><tbody><tr v-for="row in breakdown.items" :key="row.id" :class="{ 'clickable-row': dimension !== 'costRule' }" @click="dimension !== 'costRule' && drill(row)"><td><strong>{{ row.name }}</strong><small>{{ row.id }}</small></td><td>{{ row.requests.toLocaleString() }}</td><td>{{ Number(row.totalTokens).toLocaleString() }}</td><td>{{ formatCny(row.costUsd) }}</td><td>{{ (row.successRate * 100).toFixed(1) }}%</td><td>{{ row.p95LatencyMs ?? '—' }}ms</td><td v-if="dimension === 'costRule'"><strong>in {{ formatCny(row.avgInputPerMillion) }} / out {{ formatCny(row.avgOutputPerMillion) }}</strong><small>{{ schedule(row.schedule) }}</small></td></tr></tbody></table><p v-if="!breakdown.items.length" class="empty">当前维度没有数据</p></div></section>
</template>
