...
<template>
  ...
  <section v-else class="panel table-panel"><table v-if="rows.items.length" class="usage-groups-table"><thead><tr><th>组 / 类型</th><th>状态</th><th>成员 / 模型 / Key</th><th>当前周期</th><th>额度 / 已用</th><th>预占 / 可用</th><th>消耗进度</th></tr></thead><tbody>
    <tr v-for="g in rows.items" :key="g.id">
      <td><button class="back-link" @click="router.push(`/usage-groups/${g.id}`)">{{ g.name }}</button><small>{{ g.type === 'REGION' ? '区域组' : g.type === 'PROJECT' ? '历史项目组' : '部门组' }}</small><span v-if="g.projects?.length" class="muted">{{ g.projects.map(project => project.name).join(' · ') }}</span></td>
