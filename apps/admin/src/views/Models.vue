...
<template><header class="page-header"><div><p>PUBLIC CATALOG</p><h1>公共模型目录</h1><span class="subtitle">面向员工客户端的稳定模型 ID，以及背后的渠道供应情况</span></div><div class="actions"><button @click="load">刷新</button><button class="primary" @click="createOpen = true">创建公共模型</button></div></header>
  ...
  <section v-else class="panel table-panel"><table v-if="visible.length"><thead><tr><th>公共模型</th><th>发布状态</th><th>渠道供应</th><th>健康</th><th>采购成本范围</th><th>近 24h 使用</th><th>上下文</th></tr></thead><tbody><tr v-for="model in visible" :key="model.id" class="clickable-row" @click="router.push(`/models/${encodeURIComponent(model.id)}`)"><td><strong>{{ model.displayName }}</strong><small class="mono">{{ model.id }}</small></td><td><span class="chip" :class="{ success: model.enabled }">{{ model.enabled ? '已发布' : '草稿' }}</span></td><td><strong>{{ model.abilities.filter(item => item.enabled).length }}</strong><small>共 {{ model.abilities.length }} 个渠道模型</small></td><td><StatusBadge :status="health(model)" /></td><td>{{ costRange(model) }}</td><td><strong>{{ model.usage24h.requests.toLocaleString() }} 次</strong><small>{{ model.usage24h.tokens.toLocaleString() }} Token · ${{ model.usage24h.costUsd }}</small></td><td>{{ model.contextSize?.toLocaleString() || '—' }}</td></tr></tbody></table><p v-else class="empty">没有公共模型</p></section>
