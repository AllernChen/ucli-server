...
<template>
  ...
  <template v-else-if="user"><div class="detail-grid"><article class="panel metric-block"><span>当前组织成员状态</span><strong class="small-strong">{{ user.status === 'ACTIVE' ? '正常' : '已禁用' }}</strong></article><article class="panel metric-block"><span>角色</span><strong class="small-strong">{{ user.role }}</strong></article><article class="panel metric-block"><span>设备</span><strong>{{ user.deviceCount }}</strong></article><article class="panel metric-block"><span>授权</span><strong>{{ user.deviceGrantCount }}</strong></article></div>
    <section class="panel table-panel"><div class="section-header"><div><h2>设备授权</h2><p class="muted">授权链接完整内容只在创建完成时展示一次。</p></div><button type="button" @click="router.push('/device-grants')">管理全部授权</button></div><table v-if="user.deviceGrants.length"><thead><tr><th>链接提示</th><th>状态</th><th>有效期</th><th>绑定设备</th><th>绑定时间</th><th>创建时间</th></tr></thead><tbody><tr v-for="grant in user.deviceGrants" :key="grant.id"><td class="mono">{{ grant.currentLink?.secretHint || '—' }}</td><td><StatusBadge :status="grant.status" /><small>{{ detailGrantStatusLabel(grant) }}</small></td><td>{{ grant.expiresAt ? new Date(grant.expiresAt).toLocaleString() : '永久' }}</td><td>{{ grant.deviceId || '未绑定' }}</td><td>{{ grant.boundAt ? new Date(grant.boundAt).toLocaleString() : '—' }}</td><td>{{ new Date(grant.createdAt).toLocaleString() }}</td></tr></tbody></table><p v-else class="empty">暂无设备授权</p></section>
