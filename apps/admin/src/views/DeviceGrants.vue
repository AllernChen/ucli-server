...
const migration = ref<InstanceType<typeof DeviceGroupMigration> | null>(null)
...
<template>
  ...
  <DeviceGroupMigration ref="migration" @changed="load" />
  ...
  <section v-else class="grant-groups"><p v-if="error" class="state error">{{ error }}</p><article v-for="group in result.items" :key="group.id" class="panel grant-group"><div class="section-header"><div><h2>{{ group.displayName }}</h2><p class="muted">{{ group.email }}</p></div><button type="button" @click="router.push(`/users/${group.id}`)">查看用户</button></div><table><thead><tr><th>URL 提示</th><th>URL 状态</th><th>URL 有效期</th><th>授权状态</th><th>授权有效期</th><th>绑定设备</th><th>创建人</th><th>创建时间</th><th>操作</th></tr></thead><tbody><tr v-for="grant in group.deviceGrants" :key="grant.id"><td class="mono">{{ grant.currentLink?.secretHint || '未生成' }}</td><td>{{ grant.currentLink ? linkStatusLabel(grant.currentLink.status) : '未生成' }}</td><td>{{ grant.currentLink ? (grant.currentLink.expiresAt ? new Date(grant.currentLink.expiresAt).toLocaleString() : '永久') : '未生成' }}</td><td><StatusBadge :status="grant.status" /><small>{{ grantStatusLabel(grant.status) }}</small><small>{{ grant.groupName || '未归组' }}</small></td><td>{{ grant.expiresAt ? new Date(grant.expiresAt).toLocaleString() : '永久' }}</td><td>{{ grant.device?.name || '未绑定' }}<small v-if="grant.boundAt">{{ new Date(grant.boundAt).toLocaleString() }}</small></td><td class="mono">{{ grant.createdById }}</td><td>{{ new Date(grant.createdAt).toLocaleString() }}</td><td><div class="actions" @click.stop><DeviceGrantLinkActions :grant="grant" @changed="load" /><button v-if="!grant.groupId && grant.status !== 'DELETED'" :disabled="actionPending" @click="migration?.choose(grant)">首次归组</button><button v-if="grantActions(grant).includes('enable')" type="button" class="primary" :disabled="actionPending" @click="enable(grant)">启用</button><button v-if="grantActions(grant).includes('disable')" type="button" :disabled="actionPending" @click="pendingAction = { grant, action: 'disable' }">禁用</button><button v-if="grantActions(grant).includes('edit-expiry')" type="button" :disabled="actionPending" @click="openExpiry(grant)">有效期</button><button v-if="grantActions(grant).includes('delete')" type="button" class="danger-link" :disabled="actionPending" @click="pendingAction = { grant, action: 'delete' }">删除</button><span v-if="grantActions(grant).length === 0" class="muted">无可用操作</span></div></td></tr></tbody></table></article><p v-if="!result.items.length" class="panel empty">没有符合条件的授权用户</p><Pagination v-bind="result" @change="setOffset" /></section>
