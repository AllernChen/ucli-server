...
`EMPLOYEE_API_KEYS_ENABLED` 默认关闭，升级应用不会自动开启员工 Key 入口，也不会自动配置组成员、模型白名单或预算。配置、迁移与限制见[员工接入](docs/employee-api-access.md)，验收证据和发布前检查见[网关验收记录](docs/employee-gateway-acceptance.md)。
