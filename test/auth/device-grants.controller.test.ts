...
describe('device grants controller', () => {
  ...
  it('exposes and delegates expiration and lifecycle routes in the current organization', async () => {
    ...
    expect(grants.updateExpiration).toHaveBeenCalledWith(request.principal.organizationId, grantId, updateInput.expiresAt)
    expect(grants.disable).toHaveBeenCalledWith(request.principal.organizationId, grantId)
    expect(grants.enable).toHaveBeenCalledWith(request.principal.organizationId, grantId)
    expect(grants.delete).toHaveBeenCalledWith(request.principal.organizationId, grantId)
