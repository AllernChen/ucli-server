...
it('searches and pages the active option dimension through visible controls', async () => {
  ...
  await wrapper.get('[aria-label="搜索渠道选项"]').setValue(` ${hundred} `); await flushPromises()
  expect(emitted().q).toBe(hundred)
  expect(await validate(plainToInstance(UsageQueryDto, emitted()))).toHaveLength(0)
  const calls = state.api.mock.calls.length
  await wrapper.get('[aria-label="搜索渠道选项"]').setValue('x'.repeat(101)); await flushPromises()
  expect(wrapper.text()).toContain('搜索条件最多 100 个字符')
  expect(state.api.mock.calls).toHaveLength(calls)
