...
export function scanSkillEntries(entries: SkillEntry[]) {
  ...
  const files = entries.map(entry => {
    ...
    // Check declared limits before accessing lazily decompressed ZIP contents.
    if (!Number.isSafeInteger(entry.size) || entry.size < 0 || entry.size > 5 * 1024 * 1024) throw new TypeError('Skill archive entry size is invalid')
    const content = entry.content
    if (entry.size !== content.length) throw new TypeError('Skill archive entry size is invalid')
    ...
    return { path: entry.name, size: entry.size, sha256: createHash('sha256').update(content).digest('hex') }
  })
