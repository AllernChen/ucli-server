...
### 第四阶段执行记录（2026-09-15，源码及本地自动验收完成）

- Task 8 管理页面已实现：用量组列表/详情、成员管理与实际模型权限预览、当前/下周期人民币额度、预算周期和账目；用户详情代管员工 Key，个人页仅查看/撤销自己的 Key。密钥明文只在创建后显示一次，关闭、离开和切换员工均清空；不新增依赖。
- 新增经 AuthGuard 校验的 `GET /api/v1/auth/me`；页面导航与路由内容按服务端角色过滤，不使用 JWT 解码决定权限。新增个人/管理员所属组查询，只返回当前有效成员组；预算操作使用幂等 UUID，兼容内部 HTTP 浏览器环境。
- Task 8 验证：18 个管理端测试文件、148 项测试通过，真实 PG 管理接口数据用例 2 项通过；typecheck、服务端构建、管理端构建、编译产物员工 Key HTTP 测试通过（含 me 与所属组接口隔离）。保留原有 Analytics 大包告警。未部署、未开放生产 Key 入口。
- Task 9 已实现组/Key/凭据筛选、组/Key 统计维度、CNY 明确输出（旧 costUsd 为同数值兼容别名）、归属名称快照及待核对请求单列。渠道分析按实际路由成本计费、同一渠道按请求去重，历史无路由成本日志保持原最终渠道归属。组详情嵌入本组使用日志。
- 真实 PG 发现并修复原统计时间比较隐式受数据库时区影响的问题，UTC 时间范围显式转换后，¥4.00000000 固定样本在总览/组/员工/Key/模型/渠道一致，含多次路由、历史未归组、改名与未知费用。针对管理端/统计/使用/网关组合测试 159 项通过；新增嵌入日志及统计组合测试 11 项通过，typecheck、admin:build 通过。
- Task 10 已实现设备创建选组、有效未归组授权（含待绑定）统计/清单、首次归组和组织强制开关。首次归组保留旧 Token，组归属不可再修改；兑换、刷新、恢复使用重新验证要求。组织锁串行化无组签发与强制开关，映射校验/写入/审计同事务。
- 复用 DeviceGroupMigration 组件服务设备和组织页面；组列表失败不能提交新设备授权，关闭和路由切换丢弃迟到结果。迁移脚本仅允许显式本地 ucli_test 库，默认 dry-run、同组织最多 1000 条、全批原子验证，已运行 PowerShell 只读检查和真实 PG 映射演练。
- 复用已有 employee-key-http.mjs 而非新增重复 E2E 框架，真实 PG/Redis、本地 mock 上游验证三协议普通/流式及工具往返、权限/预算/日志/旧设备/撤销；两个编译产物 HTTP 脚本复跑通过。补充 Chat function 工具历史被估价校验误拒的回归修复。
- 最终 107 个文件、652 项通过（仅本地命令排除 nginx Docker 单项，maxWorkers=4），行覆盖率 95.96%、分支 84.85%；typecheck、前后端构建、许可证检查通过。完整 verify 的 nginx 用例仍因 Docker Hub 鉴权地址连接超时失败；默认高并发下另一次 PG 用例超时，4 worker 同用例 1.75 秒通过；不修改超时或降低覆盖率。
...
- 测试默认 maxWorkers=4，原样 npm run verify 连续两轮通过（653 项），未提高超时、降低覆盖率或排除 Docker/PG 用例。
- 关键路径人工复核发现人工对账补录缺失日志时保留 UNKNOWN 状态；新增 SETTLE/RELEASE 回归先失败后修复通过。最终默认 verify 为 108 文件、655 项通过，行覆盖率 95.96%，两个 HTTP E2E 通过。
- 公司部署技能只读预检确认生产 0.3.2、PG 16.14、四个服务与双健康端点正常，组预算迁移尚未应用；仅找到 9 月 2 日旧备份，缺少本次新鲜备份及隔离恢复演练。暂停生产升级，待确定 staging 与备份恢复步骤；不自动创建共享生产数据库中的演练库，不改业务开关。
- 尚未合并/推送/打包/发布。后续需要先满足 DEPLOY.md 的演练及回滚条件，固定新版本和干净源码，不能覆盖现有 0.3.2 镜像。详细证据见员工网关验收记录最新章节。

### 公司服务器隔离恢复演练（2026-09-15 12:30）

- 用户确认独立临时 PG 方案后，完成新鲜生产备份、同版本 PG 独立容器恢复及组预算迁移；23 张历史表原有列的行数和内容指纹保持一致。
- 提交 67f8151 的独立演练镜像启动 API/Gateway，专用 Redis、随机测试密钥、internal 网络且不映射端口。小组/成员/人民币预算及设备 preview/redeem/refresh/bootstrap/目录/停用 HTTP 验证通过，不调用付费上游、不启动 Worker。
- 演练容器均已停止，所有备份/数据/镜像/证据保留于 /data/ucli-server 下，等待确认清理。生产 0.3.2 镜像与配置未变，双健康检查通过。
- 构建和官方 npm audit 发现 10 high、5 moderate 的依赖告警（包含传递归因），未自动修复。下一步需核查安全告警，再固定正式版本、合并/打包/发布；本次仅完成隔离演练，不宣布正式发布收尾完成。完整路径、哈希和限制见 docs/employee-gateway-acceptance.md 最新章节。

### 演练后依赖修复（2026-09-15）

- 定向更新 adm-zip、multer、Swagger/js-yaml、qs，保留 NestJS/Prisma/MinIO 的现有主版本；官方生产依赖审计由 15 项降至 7 项，剩余三类底层告警的静态可达性和后续要求见 docs/dependency-security-review.md，未自动接受风险。
- 回归先复现 ZIP 在扫描限额前分配内存的问题，再将解压延后到扫描器检查之后。真实上传 HTTP、安全回归、类型检查和双端构建通过；Docker 测试发现并排除本地 data 构建上下文，网络失败后单项重试通过。
- 本地 PG 未运行且启动命令被执行策略拦截，已请求操作者启动；新代码的全量数据库/预算回归仍待执行，不替代为生产库测试，不宣称本次已正式发布。

### 安全修复后的全量复验（2026-09-15 13:06）

- 已启动本地专用测试 PG/Redis；根据日志修正连接用户为 ucli_test，16 条迁移无待执行项，不改数据库账户或迁移脚本。
- 原样 npm run verify 全部通过：108 文件、656 项，无跳过，行/语句 95.96%、分支 85.12%、函数 97.77%；三条 HTTP 回归和双端构建/许可证检查通过。全量验证阻塞解除，不复用旧版测试结果。
- 收尾正常停止本轮测试实例、保留数据。剩余 7 项依赖告警仍待修复或明确风险接受；未合并/推送/生产部署。最新证据见 docs/dependency-security-review.md。

### Prisma 依赖修复与复验（2026-09-15 13:21）

- 保留 Prisma 6.19.3，定向覆盖 deepmerge-ts 8.0.0。循环引用回归先复现旧版栈溢出再通过，新回归同时验证真实 Prisma 配置加载；独立只读审查未发现实质问题。
- 干净安装、生成、16 条迁移检查通过；原样 npm run verify 为 109 文件、658 项全过，无跳过，行/语句 95.96%。三条 HTTP 回归、双端构建及许可证检查通过，测试 PG/Redis 正常停止并保留数据。
- 生产依赖审计从 7 项降至 4 moderate（0 high），剩余均为 MinIO 链。最新 MinIO SDK 尚无兼容修复，不强制覆盖 ESM/旧 API、不降级或隐藏告警；继续清零需确认对象存储客户端替换范围或等待上游修复。没有接受风险、提交/合并/推送或部署。

### S3 客户端替换与本地验证（2026-09-15 14:29）

- 用户批准后移除 minio 客户端，使用 @aws-sdk/client-s3 3.1132.0；保留配置、桶、对象 Key、Region 发现及流式下载。MinIO 服务与生产数据没有变动。
- 新增 6 项默认/自定义 Region 真实 MinIO 回归，技能 HTTP 脚本改用真实独立 MinIO 并补下载。干净安装后的完整 verify 为 110 文件、664 项通过，无跳过；覆盖率行/语句 95.96%。三条 HTTP 回归、双端构建和 452 条许可证检查通过；独立审查未发现实质问题。
- 生产依赖 audit 为 0，开发工具仍有 4 项告警，未默认接受。临时 MinIO 测试数据已清理，PG/Redis 正常停止并保留数据。尚未提交/合并/推送、制作正式包或生产发布；证据见 docs/dependency-security-review.md。
