...
**已部署服务端基线：** UCLI Server `0.3.1`，Skills catalog 数字序列化修复合并提交 `aa748c4b7488302507346f86e6a2f51ccbba0c10`，2026-08-30 已部署并验证。

**模型协议能力最低源代码基线：** UCLI Server `0.3.0` 源代码提交 `f966d60c6f6d5d313b8cc8646604eb1e6b99387b`，或包含该提交的后续发布。该基线表示本文模型能力合同已进入源代码，不表示已部署到生产环境。

**模型协议能力上线状态：** 服务端生产部署、Responses 通道与映射配置、UCLI 客户端实现和全链路 live smoke 已于 2026-08-30 完成；模型流、Skills catalog 与下载 SHA-256 校验均通过。
...
## 员工 API Key 补充（源码第二阶段，未部署）

服务端增加独立员工 Key 的网关接入，详见 [员工 API Key 接入](employee-api-access.md)。此能力不替换 UCLI 的设备授权、刷新或 bootstrap 协议；员工 Key 仅用于网关，不能调用控制面、刷新会话或 bootstrap。已有设备无需因本次源码补充重新授权。当前 Key 网关开关默认关闭，预算控制尚未接入。
