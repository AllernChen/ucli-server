...
早期 `npm run verify` 被 Docker Hub `auth.docker.io` 连接超时阻断；用户补齐镜像后，该阻塞已解除，最新全量结果见下节。默认并发的 group-gateway 5 秒超时仍可复现，不能把受控并发通过写成默认 `npm run verify` 全绿。没有调低覆盖率、修改测试超时或永久屏蔽用例。
